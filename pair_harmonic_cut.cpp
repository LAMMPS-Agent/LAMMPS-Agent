...
/* ----------------------------------------------------------------------
   ...
   modeling of bacteria film. Last modified date: 10/03/2020
------------------------------------------------------------------------- */
  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    for (jj = 0; jj < jnum; jj++) {
      ...
      if (rsq < cutsq[itype][jtype]) {
        ...
        fpair = -epsilon[itype][jtype] * (sqrt(rsq) - sigma[itype][jtype]);

        f[i][0] += delx*fpair/sqrt(rsq);
        f[i][1] += dely*fpair/sqrt(rsq);
        f[i][2] += delz*fpair/sqrt(rsq);
        if (newton_pair || j < nlocal) {
          f[j][0] -= delx*fpair/sqrt(rsq);
          f[j][1] -= dely*fpair/sqrt(rsq);
          f[j][2] -= delz*fpair/sqrt(rsq);
        }
        ...
        if (eflag) {
          ...
          evdwl = epsilon[itype][jtype] * pow(sqrt(rsq) - sigma[itype][jtype], 2);
        }
...
void PairHarmonicCut::compute_inner()
{
  int i,j,ii,jj,inum,jnum,itype,jtype;
...
void PairHarmonicCut::compute_middle()
{
  int i,j,ii,jj,inum,jnum,itype,jtype;
  double xtmp,ytmp,ztmp,delx,dely,delz,fpair;
  double rsq,r2inv,r6inv,forcelj,factor_lj,rsw;
...
void PairHarmonicCut::compute_outer(int eflag, int vflag)
{
  int i,j,ii,jj,inum,jnum,itype,jtype;
  double xtmp,ytmp,ztmp,delx,dely,delz,evdwl,fpair;
  double rsq,r2inv,r6inv,forcelj,factor_lj,rsw;
