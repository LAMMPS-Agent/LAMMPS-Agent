...
static const char cite_pair_fluidmembrane[] =
  "pair fluidmembrane command:\n\n";

/* ---------------------------------------------------------------------- */

Pairfluidmembrane::Pairfluidmembrane(LAMMPS *lmp) : Pair(lmp)
{
  if (lmp->citeme) lmp->citeme->add(cite_pair_fluidmembrane);
...
Pairfluidmembrane::~Pairfluidmembrane()
{
  if (allocated) {
...
void Pairfluidmembrane::compute(int eflag, int vflag)
{
  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    for (jj = 0; jj < jnum; jj++) {
      ...
      if (rsq < cutsq[itype][jtype]) {
        ...
        one_eng = fluidmembrane_analytic(i,j,a1,a2,r12,rsq,fforce,ttor,rtor);
...
void Pairfluidmembrane::allocate()
{
  allocated = 1;
  int n = atom->ntypes;
...
void Pairfluidmembrane::settings(int narg, char **arg)
{
...
void Pairfluidmembrane::coeff(int narg, char **arg)
{
  if (narg < 8 || narg > 8){
    printf("nargs: %d\n", narg);
...
void Pairfluidmembrane::init_style()
{
...
double Pairfluidmembrane::init_one(int i, int j)
{
...
void Pairfluidmembrane::write_restart(FILE *fp)
{
...
void Pairfluidmembrane::read_restart(FILE *fp)
{
...
void Pairfluidmembrane::write_restart_settings(FILE *fp)
{
...
void Pairfluidmembrane::read_restart_settings(FILE *fp)
{
  int me = comm->me;
  if (me == 0) {
...
void Pairfluidmembrane::write_data(FILE *fp)
{
...
void Pairfluidmembrane::write_data_all(FILE *fp)
{
  for (int i = 1; i <= atom->ntypes; i++)
...
double Pairfluidmembrane::fluidmembrane_analytic(const int i,const int j,double a1[3][3],
